"""
detect_gated.py -- score-gated decode variant of detect.py (side-by-side copy).

WHAT IS DIFFERENT FROM detect.py
--------------------------------
1. decode_v8() is "gated": it checks the cheap classification score FIRST and only
   runs the expensive DFL box-softmax on the cells that pass the confidence
   threshold. The original decodes the box for ALL ~8400 cells and then throws
   ~99% away. The output is bit-for-bit identical (proven by parity_check below in
   the PR) -- this is a pure compute reordering, NOT a precision/accuracy change.
   On a clear-sky frame, zero cells pass, so the whole softmax is skipped.

2. Honest timing. The original HUD printed "infer FPS" = 1000/infer_ms, which only
   counts the Hailo call and OVERSTATES the real rate. This file additionally
   measures:
       - decode_ms : time inside decode_v8() alone (this is what the gate shrinks)
       - frame_ms  : true wall-clock per frame (capture -> draw), i.e. REAL FPS
   and shows REAL FPS next to the inference-only number so they can be compared.

The original detect.py is kept unchanged so the two can be run on the same clip and
compared. Everything else (letterbox, foveation, NMS, unwarp, recording) is identical.
"""
...
def decode_v8(outputs, conf_thres):
    # Group the raw HEF tensors by feature-map size into a box(64ch)/cls(1ch) pair.
    feats = {}
    ...
    # --- THE GATE --------------------------------------------------------------
    # The score is sigmoid(cls_logit) and sigmoid is strictly monotonic, so
    #     sigmoid(logit) >= conf   <=>   logit >= ln(conf/(1-conf))
    # i.e. we can translate the threshold into logit-space ONCE and then test the
    # raw logits with a single cheap comparison -- no exp, no sigmoid over all
    # 8400 cells. The -1e-3 slack makes the pre-filter PERMISSIVE: it can never
    # drop a borderline cell; the exact `score >= conf_thres` test on the few
    # survivors below is what actually decides each box. Net result is identical
    # to the original decode, but the expensive box-softmax only runs on cells
    # that could plausibly be a detection (zero of them on a clear-sky frame).
    logit_thr = float(np.log(conf_thres / (1.0 - conf_thres))) - 1e-3
    ...
    for size, fc in sorted(feats.items()):
        ...
        cls_logit = fc["cls"].reshape(-1).astype(np.float32)
        keep = cls_logit >= logit_thr
        if not keep.any():
            # clear-sky fast path: nothing here could exceed the threshold, so we
            # never touch this scale's box tensor at all (skips its whole softmax).
            continue

        # DFL distance decode -- ONLY for the surviving cells. box[keep] selects
        # just their 4x16 distributions; softmax over the 16 bins then a weighted
        # sum with [0..15] gives the (left, top, right, bottom) distances.
        box = fc["box"].reshape(-1, 4, DFL_BINS)[keep].astype(np.float32)
        dist = (_softmax(box, axis=-1) * bins).sum(-1)        # (M,4) l,t,r,b

        # anchor centre of each surviving cell (grid index + 0.5), masked the same
        # way so it lines up row-for-row with `box`/`dist`.
        gy, gx = np.meshgrid(np.arange(size), np.arange(size), indexing="ij")
        ax = gx.reshape(-1).astype(np.float32)[keep] + 0.5
        ay = gy.reshape(-1).astype(np.float32)[keep] + 0.5
        ...
        score = _sigmoid(cls_logit[keep])
        exact = score >= conf_thres   # identical to the original score threshold
        boxes_all.append(np.stack([x1, y1, x2, y2], axis=1)[exact])
        scores_all.append(score[exact])
...
def run_frame(model, frame, fov, conf, iou):
    ...
    # Time decode_v8() on its own -- this is exactly the stage the gate shrinks,
    # so comparing this number against the original detect.py shows the win.
    ...
    return boxes, scores, infer_ms, decode_ms, outputs


def draw(frame, boxes, scores, infer_ms, decode_ms, frame_ms):
    ...
    real_fps = 1000.0 / max(frame_ms, 1e-6)
    # Line 1: IDENTICAL to detect.py's HUD -- same text, position and colour, so
    # the inference readout matches the original one-to-one for comparison.
    cv2.putText(frame, f"infer {infer_ms:4.1f}ms  ({infer_fps:5.1f} FPS)",
                (10, 25), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 255, 255), 2)
    # Line 2 (new): the stage the gate shrinks (decode_ms) + the HONEST
    # end-to-end rate (REAL FPS from frame_ms). The original detect.py only ever
    # showed the inference-only FPS on line 1, which overstates the true rate.
    cv2.putText(frame,
                f"decode {decode_ms:4.1f}ms   REAL {real_fps:5.1f} FPS ({frame_ms:4.1f}ms)",
...
def run_images(model, fov, args):
    ...
    for p in imgs:
        ...
        if frame is None:
            ...
            continue
        boxes, scores, infer_ms, decode_ms, outputs = run_frame(
        ...
        print(f"{p.name}: {len(boxes)} det  infer {infer_ms:.1f}ms  "
              f"decode {decode_ms:.2f}ms  {dets}")
        if args.save:
            draw(frame, boxes, scores, infer_ms, decode_ms, infer_ms + decode_ms)
            out = Path(args.save)
...
def run_camera(model, fov, args):
    ...
    print(f"camera: {args.camera}  -- Ctrl+C / 'q' to stop  [GATED build]")

    # mp4 recording (open lazily on the first frame so we know W,H). The _gated
    # suffix keeps these recordings separate from the original detect.py ones.
    writer, out_path = None, None
    if not args.no_record:
        out_dir = Path(args.out_dir)
        out_dir.mkdir(parents=True, exist_ok=True)
        stamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        out_path = out_dir / f"telescope_hailo_gated_{stamp}.mp4"

    n = 0
    infer_ms_s = decode_ms_s = frame_ms_s = 0.0
    ...
    try:
        while True:
            ...
            # true end-to-end frame time = wall clock between consecutive frames
            # (capture + pre-process + infer + decode + nms + draw). This is the
            # honest throughput; infer_ms alone is NOT.
            now = time.perf_counter()
            frame_ms = (now - t_prev) * 1000.0
            t_prev = now

            # smooth all three so the on-screen numbers are steady.
            infer_ms_s = infer_ms if n == 0 else 0.9 * infer_ms_s + 0.1 * infer_ms
            decode_ms_s = decode_ms if n == 0 else 0.9 * decode_ms_s + 0.1 * decode_ms
            frame_ms_s = frame_ms if n == 0 else 0.9 * frame_ms_s + 0.1 * frame_ms

            # Always draw before recording so the saved mp4 has the overlays.
            draw(frame, boxes, scores, infer_ms_s, decode_ms_s, frame_ms_s)
            if writer is not None:
                writer.write(frame)

            if args.no_display:
                print(f"f{n:05d}  infer {infer_ms_s:5.1f}ms  "
                      f"decode {decode_ms_s:5.2f}ms  "
                      f"REAL {1000.0 / max(frame_ms_s, 1e-6):5.1f} FPS  "
                      f"{len(boxes)} det")
            else:
                cv2.imshow("telescope Hailo-8 detection [GATED]", frame)
...
def main():
    ...
    print(f"loading HEF: {args.hef}  [GATED decode]")
