...
from lucas_kanade.lk_tracker import LKSingleTargetTracker, LKState

HEF_PATH     = REPO / "yolov8n.hef"
CLASSES      = ["aircraft"]
MODEL_SIZE   = 640
CONF         = 0.5
MATCH_RADIUS = 200.0
...
def nearest_detection(det_bboxes, ref_bbox, max_dist):
    if not det_bboxes or ref_bbox is None:
        return None
    rcx, rcy = bbox_center(ref_bbox)
    best, best_d = None, max_dist
    for db in det_bboxes:
        dcx, dcy = bbox_center(db)
        d = ((dcx - rcx) ** 2 + (dcy - rcy) ** 2) ** 0.5
        if d < best_d:
            best, best_d = db, d
    return best
...
def main():
    ...
    tracker = LKSingleTargetTracker(estimate_scale=False)
            ...
            with network_group.activate(ng_params):
                try:
                    while True:
                        ...
                        # Preprocess + inference on the Hailo-8.
                        lb, scale, padx, pady = letterbox(frame, MODEL_SIZE)
                        rgb = cv2.cvtColor(lb, cv2.COLOR_BGR2RGB)
                        inp = np.expand_dims(rgb, axis=0)
                        results = pipeline.infer({in_info.name: inp})
                        dets = parse_nms(results[out_info.name],
                                         scale, padx, pady, W, H, args.conf)

                        det_bboxes = [(d[0], d[1], d[2], d[3]) for d in dets]
                        confs      = [d[4] for d in dets]

                        # Detection -> LK fallback state machine.
                        driven_by = None
                        if tracker.state == LKState.no_target:
                            if det_bboxes:
                                best = det_bboxes[int(np.argmax(confs))]
                                if tracker.update_from_detection(gray, best):
                                    driven_by = "det"
                        else:
                            match = nearest_detection(det_bboxes, tracker.bbox,
                                                      MATCH_RADIUS)
                            if match is not None:
                                tracker.update_from_detection(gray, match)
                                driven_by = "det"
                            elif prev_gray is not None:
                                tracker.update(prev_gray, gray)
                                driven_by = "lk"

                        # Draw the locked target's box.
                        if tracker.state == LKState.locked and tracker.bbox is not None:
                            x1, y1, x2, y2 = (int(v) for v in tracker.bbox)
                            color = (0, 255, 0) if driven_by == "det" else (0, 165, 255)
                            cv2.rectangle(frame, (x1, y1), (x2, y2), color, 2)
                            cv2.putText(frame, "DET" if driven_by == "det" else "LK",
                                        (x1, max(0, y1 - 6)),
                        ...
                        if args.no_show:
                            print(f"\r{fps:6.1f} FPS   state={state}   ",
                                  end="", flush=True)
                        else:
                            cv2.putText(frame, f"{fps:5.1f} FPS  |  {state}",
                                        (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 1,
                                        (0, 255, 0), 2)
                            cv2.imshow("Hailo-8 tracking - live", frame)
